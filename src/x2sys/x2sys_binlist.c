...
EXTERN_MSC int GMT_x2sys_binlist (void *V_API, int mode, void *args) {
	...
	for (trk = 0; trk < n_tracks; trk++) {
		...
		if ((error = (s->read_file) (GMT, trk_name[trk], &data, s, &p, &GMT->current.io, &row)))
			Return (error);
		...
		for (row = 0; row < p.n_rows; row++) {
			...
			if ((error = x2sys_bix_get_index (GMT, data[s->x_col][row], data[s->y_col][row], &this_bin_col,
			...
			if (this_bin_index == last_bin_index) {	/* Same bin, keep adding up incremental distances (this adds 0 the very first time) */
				...
				for (curr_x_pt = 1, prev_x_pt = 0; curr_x_pt < nx; curr_x_pt++, prev_x_pt++) {	/* Process the intervals, getting mid-points and using that to get bin */
					...
					if ((error = x2sys_bix_get_index (GMT, x, y, &ii_notused, &jj_notused, &B, &index), ""))
						Return (error);
