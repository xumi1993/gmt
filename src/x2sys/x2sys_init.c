...
EXTERN_MSC int GMT_x2sys_init (void *V_API, int mode, void *args) {
	...
	if ((error = x2sys_fclose (GMT, def_file, fp), def_file))
		Return (error);
	...
	if ((error = x2sys_fclose (GMT, track_file, fp), track_file))
		Return (error);
	...
	if ((error = x2sys_fclose (GMT, bin_file, fp), bin_file))
		Return (error);
	...
	if ((error = x2sys_fclose (GMT, path_file, fp), path_file))
		Return (error);
