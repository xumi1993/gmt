...
EXTERN_MSC int GMT_x2sys_datalist (void *V_API, int mode, void *args) {
	...
	if (Ctrl->F.flags && (error = x2sys_pick_fields (GMT, Ctrl->F.flags, s), "-F"))	/* Determine output order of selected columns */
		Return (error);
	...
	for (trk_no = 0; trk_no < n_tracks; trk_no++) {	/* Process each track */
		...
		if ((error = (s->read_file) (GMT, trk_name[trk_no], &data, s, &p, &GMT->current.io, &row)))
			Return (error);
