...
/*----------------------------------END OF VARIABLE DECLARATIONS---------------------------------------------*/
	...
	if ((error = x2sys_set_system (GMT, Ctrl->T.TAG, &s, &Bix, &GMT->current.io), Ctrl->T.TAG))
		Return (error);
	if (!s->geographic) {
	...
	for (A = 0; A < n_tracks; A++) {	/* Loop over all files */
		...
		if ((error = (s->read_file) (GMT, trk_name[A], &data[SET_A], s, &data_set[SET_A], &GMT->current.io, &n_rec[SET_A])))
			Crashout (error);
		...
		for (B = A; B < n_tracks; B++) {
			...
			if (same) {	/* Just set pointers */
				...
				if ((error = (s->read_file) (GMT, trk_name[B], &data[SET_B], s, &data_set[SET_B], &GMT->current.io, &n_rec[SET_B])))
					Crashout (error);
