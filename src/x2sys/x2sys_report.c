...
EXTERN_MSC int GMT_x2sys_report (void *V_API, int mode, void *args) {
	...
	if (Ctrl->C.col && (error = x2sys_pick_fields (GMT, Ctrl->C.col, s), "-C"))
		Return (error);
	...
	if ((snp = x2sys_read_coe_dbase (GMT, s, Ctrl->In.file, Ctrl->I.file, GMT->common.R.wesn, Ctrl->C.col, coe_kind, Ctrl->S.file, &P, &nx, &n_tracks)) < 0)
		Return (-snp);
	np = (uint64_t)snp;
